  ...
  <div class="miner-table-wrap">
    <div class="table-header">
      <div class="header-left">
        <span class="total-info">全网共 {{ total }} 台矿机在线</span>
        <div class="search-box">
          <input type="text" v-model="searchQuery" @input="onSearch" placeholder="搜索矿机名..." class="search-input" />
        </div>
      </div>
    ...
    <table class="miner-table">
      ...
      <tbody>
        ...
        <tr v-for="miner in miners" :key="miner.id + '-' + miner.port" :class="{ 'offline-row': miner.isOffline }">
          <td class="coin-cell">
            <div class="small-coin-wrapper">
              <CoinIcon :coin="miner.coinName || 'UNK'" />
            </div>
          </td>
          <td><code>{{ miner.port }}</code></td>
...
const fetchMiners = async () => {
  try {
    const res = await fetch(`/api/miners?port=0&page=${page.value}&limit=${limit.value}&search=${encodeURIComponent(searchQuery.value)}`)
    if (res.ok) {
