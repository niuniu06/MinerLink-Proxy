...
5. **强制深度推理 (Mandatory Chain-of-Thought)**：
   面对复杂架构修改或排查深层 Bug 时，【绝对禁止】直接给出结论或跳跃性抛出代码。你必须遵循“第一性原理”，在你的思考或回答中展示结构化的推导链条：**问题现象本质 -> 历史业务约束核对 (如F2Pool免重启、无锁架构) -> 至少2种可行方案对比 -> 潜在的连带破坏风险评估 -> 最终方案代码**。宁可分批次输出，也绝不允许敷衍简略。