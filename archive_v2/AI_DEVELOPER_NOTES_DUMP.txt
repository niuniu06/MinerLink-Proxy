...
## 2026-07-01: GitHub CLI (gh) 发布报 401 权限失败坑点记录
- **现象**：在使用 gh release create 等命令尝试向官方仓库推发布包时，即使本地已经用 gh auth login 成功登录，依然报错 HTTP 401 Unauthorized，提示认证失败。
- **真相**：用户的本地系统环境变量中，可能残留着一个早已失效或者没有对应权限的全局变量 GITHUB_TOKEN。GitHub CLI (gh) 拥有一个极其霸道的**高优先级机制**：只要它在当前终端环境里检测到 GITHUB_TOKEN，就会强行屏蔽并覆盖掉保存在本地系统凭据库里的合法登录状态，去使用这个废弃的变量。
- **修复与防呆指南**：未来在执行发版脚本或手动调用 gh 发布时，如果遇到莫名其妙的 401 权限问题，**第一步操作必须是清理环境变量**：
  在 PowerShell 中运行 Remove-Item Env:\GITHUB_TOKEN -ErrorAction SilentlyContinue
  或者在发版脚本（如 upload.ps1）的头部强行注入环境变量抹除指令，确保 gh 能够正确且优先地调用本地凭据登录，从而实现秒发成功。
